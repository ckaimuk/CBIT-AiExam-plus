...
# 导入模型
from models import db, Student, Exam, Question, Answer, ExamSession, ExamConfig, ExamConfigQuestion, ExamQuestion, StudentExamRecord, ExamTemplate, ExamTemplateQuestion, ExamInstance, StudentExam, StudentAnswer, VerificationConfig, SystemConfig, ApiProvider
...
def get_ai_models():
    ...
    try:
        ...
        models = {
            'openrouter': [
                {'id': 'openai/gpt-4-turbo-preview', 'name': 'GPT-4 Turbo', 'recommended': True},
                {'id': 'anthropic/claude-3-sonnet', 'name': 'Claude-3 Sonnet', 'recommended': True},
                {'id': 'google/gemini-pro', 'name': 'Gemini Pro', 'recommended': False},
                {'id': 'meta-llama/llama-2-70b-chat', 'name': 'Llama-2 70B', 'recommended': False},
                {'id': 'openai/gpt-3.5-turbo', 'name': 'GPT-3.5 Turbo', 'recommended': False},
                {'id': 'anthropic/claude-3-haiku', 'name': 'Claude-3 Haiku', 'recommended': False}
            ],
            'openai': [
            ...
            'google': [
                ...
                {'id': 'gemini-pro-vision', 'name': 'Gemini Pro Vision', 'recommended': False}
            ],
            'custom': [
        ...
        return jsonify({
            'success': True,
            'models': models.get(provider, models['openrouter'])
        })
...
def create_question():
    ...
    try:
        ...
        # 验证必填字段
        required_fields = ['subject', 'difficulty', 'cognitive_level', 'question_type', 'content']
...
def delete_question(question_id):
    ...
    try:
        ...
        print(f"关联记录: 模板题目({template_questions_count}), 配置题目({config_questions_count}), 考试题目({exam_questions_count}), 学生答案({student_answers_count})")
...
def ai_generate_questions():
    ...
    try:
        ...
        if count > 20:
            return jsonify({'success': False, 'message': '单次生成题目数量不能超过20道'}), 400
...
def _generate_mock_questions(count, subject, sub_tag, difficulty, cognitive_level, types, language='zh', custom_prompt=''):
    """生成模拟题目数据"""
    mock_questions = []
...
def create_exam_config():
    ...
    try:
        ...
        if question_selection_mode == 'manual' and not data.get('question_ids'):
            return jsonify({'success': False, 'message': '手动选择模式下必须选择题目'}), 400
...
def get_config_questions(config_id):
    ...
    try:
        ...
        if config.question_selection_mode == 'manual':
            # 手动选择模式：返回精确选择的题目
            config_questions = ExamConfigQuestion.query.filter_by(config_id=config_id).order_by(ExamConfigQuestion.question_order).all()
            ...
            questions = [{'question': q.to_dict(), 'points': 1.0, 'question_order': i+1} for i, q in enumerate(questions_list)]
...
def get_student_record_detail(record_id):
    ...
    try:
        ...
        # 获取该考试的题目和答案
        exam_questions = db.session.query(ExamQuestion, Question).join(
            Question, ExamQuestion.question_id == Question.id
        ).filter(ExamQuestion.exam_id == record.exam_id).all()
...
def get_student_records_statistics():
    ...
    try:
        ...
        status_stats = db.session.query(
            StudentExamRecord.status,
            db.func.count(StudentExamRecord.id)
        ).group_by(StudentExamRecord.status).all()
        
        # 平均分
        avg_score = db.session.query(db.func.avg(StudentExamRecord.total_score)).filter(
            StudentExamRecord.status == 'completed'
        ).scalar() or 0
        
        # 最高分
        max_score = db.session.query(db.func.max(StudentExamRecord.total_score)).filter(
            StudentExamRecord.status == 'completed'
        ).scalar() or 0
        
        # 最低分
        min_score = db.session.query(db.func.min(StudentExamRecord.total_score)).filter(
            StudentExamRecord.status == 'completed'
        ).scalar() or 0
...
def get_exam_template(template_id):
    ...
    try:
        ...
        # 获取模板题目
        template_questions = ExamTemplateQuestion.query.filter_by(
            template_id=template_id
        ).order_by(ExamTemplateQuestion.question_order).all()
...
def batch_delete_exam_records():
    ...
    try:
        ...
        if not exam_ids:
            return jsonify({'success': False, 'message': '未提供要删除的考试记录ID'}), 400
...
def cleanup_students():
    ...
    try:
        ...
        print(f"🧹 开始清理学生数据...")
        print(f"当前数据统计: 学生({students_count}), 会话({sessions_count}), 考试({exams_count}), 实例({instances_count}), 答案({answers_count}), 记录({records_count})")
...
def get_available_exam_templates():
    ...
    try:
        ...
        active_configs = ExamConfig.query.filter_by(is_active=True).order_by(
        ...
        for template in templates:
            ...
            if template.questions:
                try:
                    ...
                    questions_count = len(questions_list) if isinstance(questions_list, list) else template.total_questions
                except:
                    questions_count = template.total_questions
...
def get_available_exam_instances():
    ...
    try:
        ...
        instances = ExamInstance.query.filter(
            ...
            or_(
                ExamInstance.end_time.is_(None),
                ExamInstance.end_time >= current_time
            )
        ).all()
...
def generate_questions_from_config(config):
    ...
    try:
        ...
        if config.question_selection_mode == 'manual':
            # 手动选择模式：使用预先选定的题目
            print("📝 使用手动选择模式")
            config_questions = ExamConfigQuestion.query.filter_by(
                config_id=config.id
            ).order_by(ExamConfigQuestion.question_order).all()
...
def generate_questions_by_filter(config):
    ...
    if len(available_questions) < config.total_questions:
        # 如果题目不够，补充其他题目
        additional_questions = Question.query.filter_by(
            is_active=True
        ).all()
    ...
    try:
        ...
        for config in active_configs:
            ...
            config_exam_info = {
                ...
                'name': config.name,
                'description': config.description or (f'基于{"默认" if config.is_default else ""}配置 "{config.name}" 的考试'),
                'time_limit': config.time_limit,
                'total_questions': config.total_questions,
                'passing_score': config.passing_score,
                'is_active': config.is_active,
                ...
                'statistics': {
                    ...
                    'avg_score': round(sum(p['percentage'] for p in old_participants) / len(old_participants) if old_participants else 0, 1)
                },
                'participants': sorted(old_participants, key=lambda x: x['completed_at'] or '1970-01-01T00:00:00', reverse=True)
            }
        ...
        for template in templates:
            ...
            avg_score = sum(i.percentage for i in completed_instances) / len(completed_instances) if completed_instances else 0
...
def get_exams_history():
    ...
    try:
        ...
        for instance in instances:
            ...
            if instance.id:
                # 通过session查找学生名称
                session_query = db.session.query(ExamSession.student_id).join(
                    ExamInstance, ExamInstance.session_id == ExamSession.id
                ).filter(ExamInstance.id == instance.id).first()
    ...
    try:
        ...
        for question_type, stats in type_stats.items():
            ...
            result_stats[question_type] = {
                ...
                'percentage': round((stats['total_score'] / stats['max_score'] * 100) if stats['max_score'] > 0 else 0, 1),
                'question_count': stats['question_count'],
                'student_count': student_count,
                'correct_count': stats['correct_count'],
                'accuracy': round((stats['correct_count'] / stats['question_count'] * 100) if stats['question_count'] > 0 else 0, 1)
            }
        
        return jsonify({
            'success': True,
            'type_scores': result_stats
        })
    ...
    try:
        ...
        for instance in instances:
            ...
            for answer in answers:
                ...
                type_stats[question_type]['detailed_scores'].append({
                    'question_id': question.id,
                    'question_text': question.content[:100] + '...' if len(question.content) > 100 else question.content,
                    'score': answer.score if answer.score else 0,
                    'max_score': question.points,
                    'percentage': round((answer.score / question.points * 100) if answer.score and question.points > 0 else 0, 1),
                    'is_correct': is_correct,
                    'exam_instance_id': instance.id,
                    'exam_date': instance.completed_at.strftime('%Y-%m-%d %H:%M') if instance.completed_at else 'N/A'
                })
        ...
        for question_type, stats in type_stats.items():
            ...
            result_stats[question_type] = {
                ...
                'correct_count': stats['correct_count'],
                'accuracy': round((stats['correct_count'] / stats['question_count'] * 100) if stats['question_count'] > 0 else 0, 1),
                'avg_score_per_question': round(stats['total_score'] / stats['question_count'], 2) if stats['question_count'] > 0 else 0,
    ...
    try:
        ...
        for student_name, student_data in all_students_data.items():
            ...
            if student_data['total_max_score'] > 0:
                student_data['total_percentage'] = round((student_data['total_score'] / student_data['total_max_score']) * 100, 1)
            else:
            ...
            for question_type, type_data in student_data['type_scores'].items():
                ...
                if type_data['question_count'] > 0:
                    type_data['accuracy'] = round((type_data['correct_count'] / type_data['question_count']) * 100, 1)
                else:
...
def delete_all_students():
    ...
    try:
        ...
        if failed_deletions:
            ...
            return jsonify({
                'success': True, 
                'message': f'成功删除所有 {deleted_count} 个学生'
            })
...
def get_dashboard_stats():
    ...
    try:
        ...
        today_exams = ExamInstance.query.filter(
            ExamInstance.created_at >= today_start,
            ExamInstance.created_at < today_end
        ).count()
        
        # 计算平均考试时长 (分钟)
        completed_exams = ExamInstance.query.filter(
            ExamInstance.completed_at.isnot(None),
            ExamInstance.created_at.isnot(None)
        ).all()
...
def get_dashboard_charts_data():
    ...
    try:
        ...
        # 获取最近的考试实例
        recent_instances = ExamInstance.query.filter(
            ExamInstance.completed_at.isnot(None)
        ).order_by(ExamInstance.completed_at.desc()).limit(10).all()
        ...
        if len(recent_activities) < 10:
            ...
            ).order_by(Exam.completed_at.desc()).limit(remaining_count).all()
...
def get_all_student_answers():
    ...
    try:
        ...
        # 从Answer表获取答题记录（新版本）
        answers_query = Answer.query.join(Question).outerjoin(
            ExamInstance, Answer.exam_instance_id == ExamInstance.id
        ).outerjoin(
            Student, ExamInstance.student_id == Student.id
        ).filter(
            Answer.exam_instance_id.isnot(None)
        ).with_entities(
            Answer.answer_text,
            Answer.is_correct,
            Answer.submitted_at,
        ...
        if len(answers_data) < per_page and page == 1:
            ...
            # 从Answer表获取旧版答题记录
            old_answers_query = Answer.query.join(Question).outerjoin(
                Exam, Answer.exam_id == Exam.id
            ).outerjoin(
                ExamSession, Exam.session_id == ExamSession.id
            ).outerjoin(
                Student, ExamSession.student_id == Student.id
            ).filter(
                Answer.exam_id.isnot(None)
            ).with_entities(
                Answer.answer_text,
                Answer.is_correct,
                Answer.submitted_at,
            ...
            ).order_by(Answer.submitted_at.desc()).limit(remaining_count)
...
def get_admin_all_student_answers_summary():
    ...
    try:
        ...
        for student in students:
            ...
            overall_avg = ((instance_avg_score * instance_count) + (exam_avg_score * exam_count)) / total_exams if total_exams > 0 else 0
        ...
        try:
            ...
            if models_response.status_code == 200:
                ...
                for model in models_data.get('data', []):
                    ...
                    # 只显示推荐的模型
                    if any(recommended in model_id.lower() for recommended in [
                        'gpt-4', 'gpt-3.5', 'claude-3', 'claude-2', 'gemini-pro', 'llama-2'
                    ]):
...
def validate_provider_api(provider_name):
    ...
    try:
        ...
        try:
            ...
            return jsonify({'success': False, 'message': f'不支持的API提供商: {provider_name}'}), 400
...
def save_provider_config(provider_name):
    ...
    try:
        ...
        if success:
            return jsonify({
                'success': True,
                'message': f'{provider_name.title()} API配置保存成功'
            })
        else:
            return jsonify({
                'success': False,
                'message': '保存配置失败，请检查API密钥是否有效'
            }), 400
...
def activate_provider(provider_name):
    ...
    try:
        ...
        if success:
            return jsonify({
                'success': True,
                'message': f'已成功激活 {provider_name.title()} API'
            })
        else:
            return jsonify({
                'success': False,
...
def get_provider_details(provider_name):
    ...
    try:
        ...
        if not provider_config:
            return jsonify({'success': False, 'message': f'{provider_name} 配置不存在'}), 404
                        ...
                        if instance.score is None:
                            try:
                                ...
                                if instance.questions:
                                    ...
                                    print(f"✅ 自动计算过期考试分数: {instance.score}/{instance.total_score} ({instance.percentage}%)")
        ...
        for answer in new_answers:
            try:
                ...
                all_answers_data.append({
                    ...
                    'submitted_at': answer.submitted_at.isoformat() if answer.submitted_at else answer.created_at.isoformat() if hasattr(answer, 'created_at') and answer.created_at else None,
