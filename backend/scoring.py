...
class ScoringSystem:
    ...
    def _load_config_from_db(self):
        ...
        try:
            # 导入这里而不是在文件顶部，避免循环导入
            from models import SystemConfig, ApiProvider
    ...
    def calculate_scores_for_instance(self, instance_id: int, questions: List[Dict], 
    ...
    def calculate_scores(self, exam_id: int, questions: List[Dict], 
                        answers: Dict[str, str]) -> Dict[str, Any]:
        """计算考试总分和详细分数"""
    ...
    def _calculate_scores_internal(self, questions: List[Dict], answers: Dict[str, str], 
                                  exam_or_instance_id: int, is_instance: bool = False) -> Dict[str, Any]:
        """内部评分方法"""
        total_score = 0
        max_score = 0
        subject_scores = {}
    ...
    def _score_short_answer(self, question: Dict, student_answer: str, max_score: float) -> Tuple[float, float]:
        ...
        if self.ai_scoring_enabled and self.api_key:
            ...
            print(f"✅ 简答题最终评分: AI{ai_similarity:.2f}*0.8 + 关键词{keyword_score:.2f}*0.2 = {final_similarity:.2f}")
        else:
            ...
            print(f"✅ 基本评分结构: 关键词{keyword_score:.2f}*0.7 + 长度{length_score:.2f}*0.2 + 逻辑{basic_logic_score:.2f}*0.1 = {final_similarity:.2f}")
    ...
    def _score_programming(self, question: Dict, student_answer: str, max_score: float) -> Tuple[float, float]:
        ...
        try:
            ...
            if self.ai_scoring_enabled and self.api_key:
                ...
                final_score = (ai_score * 0.8 + structure_score * 0.1 + syntax_score * 0.05 + execution_score * 0.05) * max_score
                print(f"✅ AI模式最终评分: AI{ai_score:.2f}*0.8 + 结构{structure_score:.2f}*0.1 + 语法{syntax_score:.2f}*0.05 + 执行{execution_score:.2f}*0.05 = {final_score:.2f}/{max_score}")
            else:
                # AI评分禁用，使用基本评分结构
                print("📝 使用基本评分结构（非AI模式）")
                ...
                print(f"✅ 基本评分结构最终评分: 结构{structure_score:.2f}*0.4 + 语法{syntax_score:.2f}*0.35 + 执行{execution_score:.2f}*0.25 = {final_score:.2f}/{max_score}")
...
"""
    ...
    def _evaluate_basic_logic(self, student_answer: str) -> float:
        ...
        # 检查逻辑连接词
        logic_words = ['因为', '所以', '由于', '因此', '然而', '但是', '首先', '其次', '最后', 
                      '例如', '比如', '总之', '综上', 'therefore', 'because', 'however', 'first']
        ...
        # 检查是否有基本结构（多个句子）
        sentences = len([s for s in student_answer.split('。') if s.strip()]) + \
                   len([s for s in student_answer.split('.') if s.strip()])
        if sentences > 1:
            score += 0.2
            ...
            for q_score in question_scores:
                ...
                if is_instance:
                    ...
                    answer = Answer.query.filter_by(
                        exam_instance_id=exam_or_instance_id,
                        question_id=question_id
                    ).first()
                else:
                    # 旧系统：通过exam_id查找
                    answer = Answer.query.filter_by(
                        exam_id=exam_or_instance_id,
                        question_id=question_id
                    ).first()
