...
class ApiManager:
    ...
    def _load_legacy_config(self, conn):
        ...
        try:
            ...
            # 查询旧的API配置
            api_key_result = conn.execute(text("SELECT config_value FROM system_configs WHERE config_key = 'openrouterApiKey'")).fetchone()
            model_result = conn.execute(text("SELECT config_value FROM system_configs WHERE config_key = 'aiModel'")).fetchone()
                ...
                if response.status_code == 200:
                    ...
                    for model in raw_models:
                        ...
                        if any(keyword in model_id.lower() for keyword in [
                            'gpt-4', 'gpt-3.5', 'gpt-35', 'claude-3', 'claude-2', 'claude',
                            'gemini', 'palm', 'llama', 'mistral', 'qwen', 'deepseek',
                            'yi-', 'chatgpt', 'o1-', 'sonnet', 'haiku', 'opus'
                        ]):
                    ...
                    if len(recommended_models) < 10:
                        ...
                        for model in raw_models:
                            ...
                            if ('openai/' in model_id or 'anthropic/' in model_id or 
                                'google/' in model_id or 'meta-llama/' in model_id):
                    ...
                    return {"success": True, "models": recommended_models}  # 返回所有筛选出的模型，不设上限
                else:
                    error_text = response.text
                    print(f"❌ OpenRouter API调用失败: {response.status_code} - {error_text}")
                    return {"success": False, "error": f"API验证失败: {response.status_code}"}
                ...
                if models_response.status_code == 200:
                    ...
                    for model in raw_models:
                        ...
                        if any(keyword in model_id for keyword in [
                            "gpt-4", "gpt-3.5", "gpt-4o", "o1-preview", "o1-mini"
                        ]):
                            # 排除一些不常用的变体
                            if not any(exclude in model_id for exclude in [
                                "instruct", "edit", "search", "similarity", "davinci", "curie", "babbage", "ada"
                            ]):
                                chat_models.append({
                                    "id": model_id,
                    ...
                    if test_response.status_code == 200:
                        ...
                        return {"success": False, "error": f"API验证失败: {test_response.status_code}"}
    ...
    def get_available_providers(self) -> List[Dict[str, Any]]:
        ...
        for provider in ApiProvider:
            ...
            print(f"📋 提供商 {provider_name}: 有配置={has_config}, 激活={is_active}, 验证={db_info.get('is_verified', False)}")
    ...
    def save_provider_config(self, provider: ApiProvider, api_key: str, model: str = None) -> bool:
        ...
        try:
            ...
            # 优先尝试使用Flask应用上下文
            try:
                from models import db, ApiProvider as ApiProviderModel
            ...
            with engine.begin() as conn:  # 使用begin()自动处理事务
                ...
                ), {"provider": provider.value}).fetchone()
