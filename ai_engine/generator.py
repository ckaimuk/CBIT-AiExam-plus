...
class QuestionGenerator:
    ...
    def __init__(self):
        ...
        self.subjects = {
            'statistics': {
                'name': '统计学',
                'weight': 0.30,
                'topics': ['描述性统计', '推断统计', '假设检验', '回归分析', '概率分布']
            },
            'calculus': {
                'name': '微积分',
                'weight': 0.25,
    ...
    def _generate_mock_exam(self, total_questions: int = 20) -> List[Dict[str, Any]]:
        ...
        mock_questions = [
            ...
            },
            {
                ...
                'options': ['lim(h→0)[f(x+h)-f(x)]/h', 'f(x+h)-f(x)', 'f\'(x)', '∫f(x)dx'],
            ...
            },
            {
                ...
                'options': ['矩阵的行数', '矩阵的列数', '线性无关行（列）的最大个数', '矩阵的行列式'],
            ...
            },
            {
                ...
                'options': ['P(A|B) = P(B|A)P(A)/P(B)', 'P(A|B) = P(A)P(B)', 'P(A|B) = P(B|A)', 'P(A|B) = P(A)+P(B)'],
    ...
    def _generate_single_question(self, subject_key: str, subject_info: Dict, 
                                difficulty: Dict, cognitive_level: Dict, 
                                question_type: Dict, question_id: int) -> Dict[str, Any]:
        """生成单个题目"""
        try:
            # 构建提示词
    ...
    def _build_generation_prompt(self, subject_info: Dict, difficulty: Dict, 
                               cognitive_level: Dict, question_type: Dict) -> str:
        """构建AI生成提示词"""
...
"""
    ...
    def _generate_mock_questions_with_prompt(self, prompt: str, count: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return self._generate_diverse_questions_with_difficulty(generation_strategy, count, difficulty, language, subject, prompt)
    
    def _generate_diverse_questions_with_difficulty(self, strategy: str, count: int, difficulty: str, language: str, subject: str, prompt: str) -> List[Dict[str, Any]]:
        """强制多样化生成器 + 真正的难度分级"""
        questions = []
        ...
        for i in range(count):
            ...
            question = self._generate_unique_question(
                strategy, i, difficulty_config, language, subject, prompt,
                used_scenario_types, used_content_patterns
            )
    ...
    def _get_difficulty_config(self, difficulty: str, language: str) -> dict:
        ...
        professional_configs = {
            # 基础难度
            '简单': {
                'steps': 1, 'concepts': 1, 'calculation_complexity': 'basic',
                'reasoning_depth': 'direct', 'points': 1, 'time_limit': 2,
                'description': '直接应用单一公式或概念' if language == 'zh' else 'Direct application of single formula or concept'
            },
            '中等': {
                'steps': 2, 'concepts': 2, 'calculation_complexity': 'intermediate',
                'reasoning_depth': 'logical', 'points': 3, 'time_limit': 4,
                'description': '两步解题，结合两个概念，需要逻辑推理' if language == 'zh' else 'Two-step problem combining concepts with logical reasoning'
            },
            '困难': {
                'steps': 4, 'concepts': 3, 'calculation_complexity': 'advanced',
                'reasoning_depth': 'analytical', 'points': 5, 'time_limit': 8,
                'description': '多步骤分析，综合多个概念，需要深入推理' if language == 'zh' else 'Multi-step analysis with integrated concepts requiring deep reasoning'
            },
            
            # 标准化考试级别
            'gre_math': {
                'steps': 3, 'concepts': 2, 'calculation_complexity': 'moderate',
                'reasoning_depth': 'strategic', 'points': 4, 'time_limit': 3,
                'trap_answers': True, 'data_sufficiency': True,
                'description': 'GRE数学推理，需要策略性思维和陷阱识别' if language == 'zh' else 'GRE quantitative reasoning with strategic thinking and trap identification'
            },
            'gmat_math': {
                'steps': 4, 'concepts': 3, 'calculation_complexity': 'high',
                'reasoning_depth': 'business_analytical', 'points': 5, 'time_limit': 4,
                'business_context': True, 'data_sufficiency': True,
                'description': 'GMAT商业数学，结合商业背景的分析推理' if language == 'zh' else 'GMAT business math with analytical reasoning in business context'
            },
            'sat_math_2': {
                'steps': 3, 'concepts': 2, 'calculation_complexity': 'advanced',
                'reasoning_depth': 'competitive', 'points': 4, 'time_limit': 2.5,
                'multiple_approaches': True,
                'description': 'SAT数学2级，竞赛水平的多路径解法' if language == 'zh' else 'SAT Math Level 2 with competitive multiple-approach solutions'
            },
            
            # 学术研究级别
            'graduate_study': {
                'steps': 6, 'concepts': 4, 'calculation_complexity': 'very_high',
                'reasoning_depth': 'theoretical', 'points': 8, 'time_limit': 15,
                'proof_required': True, 'theoretical_basis': True,
                'description': '研究生水平，需要理论证明和深度分析' if language == 'zh' else 'Graduate level requiring theoretical proof and deep analysis'
            },
            'advanced_undergraduate': {
                'steps': 4, 'concepts': 3, 'calculation_complexity': 'high',
                'reasoning_depth': 'analytical', 'points': 6, 'time_limit': 10,
                'multi_concept_integration': True,
                'description': '本科高年级，多概念综合应用' if language == 'zh' else 'Advanced undergraduate with multi-concept integration'
            },
            'competition_math': {
                'steps': 5, 'concepts': 3, 'calculation_complexity': 'creative',
                'reasoning_depth': 'innovative', 'points': 7, 'time_limit': 12,
                'creative_approach': True, 'non_standard_methods': True,
                'description': '数学竞赛水平，需要创新思路和非常规方法' if language == 'zh' else 'Math competition level requiring innovative thinking and non-standard methods'
            },
            
            # 专业应用级别  
            'engineering_applications': {
                'steps': 5, 'concepts': 4, 'calculation_complexity': 'practical_high',
                'reasoning_depth': 'applied', 'points': 6, 'time_limit': 12,
                'real_world_context': True, 'precision_required': True,
                'description': '工程应用水平，实际问题的精确求解' if language == 'zh' else 'Engineering application level with precise real-world problem solving'
            },
            'data_science': {
                'steps': 4, 'concepts': 3, 'calculation_complexity': 'algorithmic',
                'reasoning_depth': 'statistical', 'points': 5, 'time_limit': 8,
                'statistical_modeling': True, 'algorithm_design': True,
                'description': '数据科学水平，统计建模和算法设计' if language == 'zh' else 'Data science level with statistical modeling and algorithm design'
            },
            'financial_modeling': {
                'steps': 4, 'concepts': 3, 'calculation_complexity': 'financial',
                'reasoning_depth': 'quantitative', 'points': 6, 'time_limit': 10,
                'financial_context': True, 'risk_analysis': True,
                'description': '金融建模水平，量化分析和风险评估' if language == 'zh' else 'Financial modeling level with quantitative analysis and risk assessment'
            }
        }
    ...
    def _generate_unique_question(self, strategy: str, index: int, difficulty_config: dict, language: str, subject: str, prompt: str, used_types: set, used_patterns: set) -> dict:
        """生成完全唯一的题目"""
        max_attempts = 10
        for attempt in range(max_attempts):
            if strategy == "shopping_scenario":
                question = self._create_advanced_shopping_question(index, difficulty_config, language, subject, used_types, used_patterns)
            elif strategy == "investment_scenario":
                question = self._create_advanced_investment_question(index, difficulty_config, language, subject, used_types, used_patterns)
            elif strategy == "probability_statistics":
                question = self._create_advanced_statistics_question(index, difficulty_config, language, subject, used_types, used_patterns)
            elif strategy == "school_scenario":
                question = self._create_advanced_school_question(index, difficulty_config, language, subject, used_types, used_patterns)
            elif strategy == "transport_scenario":
                question = self._create_advanced_transport_question(index, difficulty_config, language, subject, used_types, used_patterns)
            else:
                question = self._create_advanced_adaptive_question(index, difficulty_config, language, subject, used_types, used_patterns)
    ...
    def _determine_generation_strategy(self, prompt: str) -> str:
        ...
        elif re.search(r'投资|investment|利息|interest|收益|profit|理财|finance', prompt, re.IGNORECASE):
        ...
        elif re.search(r'交通|transport|车|bus|taxi|速度|speed|距离|distance', prompt, re.IGNORECASE):
            return "transport_scenario"
        elif re.search(r'餐厅|restaurant|食物|food|分享|share|pizza|蛋糕|cake', prompt, re.IGNORECASE):
            return "restaurant_scenario"
        
        # 然后检查题目类型
        elif re.search(r'\d+\s*(ml|毫升|升|liters?)|mixture|ratio|mix|混合|比例', prompt, re.IGNORECASE):
        ...
        elif re.search(r'算法|algorithm|编程|programming|计算思维|computational', prompt, re.IGNORECASE):
    ...
    def _generate_mixture_problems(self, count: int, difficulty: str, language: str, subject: str) -> List[Dict[str, Any]]:
        """生成混合物问题（根据难度分级）"""
        questions = []
        ...
        for i in range(count):
            ...
            options = [f"{answer} ml", f"{answer + 50} ml", f"{answer - 30} ml", f"{answer + 100} ml"] if language == 'en' else [f"{answer}毫升", f"{answer + 50}毫升", f"{answer - 30}毫升", f"{answer + 100}毫升"]
    ...
    def _generate_scenario_problems(self, count: int, difficulty: str, language: str, subject: str) -> List[Dict[str, Any]]:
                ...
                'en': [
                    {
                        ...
                        'variables': {'money': [80, 120, 150, 200], 'item': ['gifts', 'books', 'toys', 'snacks'], 'price': [12, 18, 25, 15]},
                    ...
                    },
                    {
                        ...
                        'variables': {'food': ['pizza', 'cake', 'pie', 'sandwich'], 'total': [8, 12, 6, 10], 'people': [3, 4, 2, 5]},
                    ...
                    },
                    {
                        ...
                        'variables': {'students': [25, 30, 28, 32], 'items': [2, 3, 4, 1], 'item': ['pencils', 'notebooks', 'erasers', 'rulers']},
                    ...
                    },
                    {
                        ...
                        'variables': {'capacity': [50, 40, 60, 45], 'waiting': [120, 85, 150, 95]},
        ...
        for i in range(count):
            ...
            if 'template' in scenario_template and 'variables' in scenario_template:
                ...
                if 'money' in selected_vars and 'price' in selected_vars:
                    ...
                    answer_num = result
                    options = [f"{result}个" if language == 'zh' else f"{result} items",
                    ...
                    answer_num = result
                    options = [f"{result}块" if language == 'zh' else f"{result} pieces",
                    ...
                    answer_num = result
                    options = [f"{result}支" if language == 'zh' else f"{result} items",
                    ...
                    answer_num = result
                    options = [f"{result}辆" if language == 'zh' else f"{result} buses",
                              f"{result+1}辆" if language == 'zh' else f"{result+1} buses",
                              f"{result-1}辆" if language == 'zh' else f"{result-1} buses",
                              f"{result+2}辆" if language == 'zh' else f"{result+2} buses"]
                else:
                    # 默认选项
                    options = ["选项A", "选项B", "选项C", "选项D"] if language == 'zh' else ["Option A", "Option B", "Option C", "Option D"]
                    answer_num = "A"
            ...
            questions.append({
                'subject': subject,
                'sub_tag': f"场景应用-{scenario_template['context']}" if language == 'zh' else f"Scenario-{scenario_template['context']}",
                'language': language,
                'difficulty': difficulty,
                'question_type': 'multiple_choice',
                'content': content,
    ...
    def _generate_mathematical_modeling(self, count: int, difficulty: str, language: str, subject: str) -> List[Dict[str, Any]]:
        """生成数学建模题目"""
        questions = []
        templates = {
    ...
    def _generate_real_world_applications(self, count: int, difficulty: str, language: str, subject: str) -> List[Dict[str, Any]]:
        """生成现实世界应用题目"""
        return self._generate_scenario_problems(count, difficulty, language, subject)
    
    def _generate_computational_thinking(self, count: int, difficulty: str, language: str, subject: str) -> List[Dict[str, Any]]:
        """生成计算思维题目"""
        questions = []
        for i in range(count):
            if difficulty == '简单':
                content = "What output: for i in range(3): print(i*2)?" if language == 'en' else "输出什么：for i in range(3): print(i*2)?"
                answer = "0, 2, 4"
            elif difficulty == '困难':
                content = "Time complexity: for i in range(n): for j in range(i): process()" if language == 'en' else "时间复杂度：for i in range(n): for j in range(i): process()"
                answer = "O(n²)"
            else:
                content = "Binary search steps for 1000 elements?" if language == 'en' else "对1000元素二分查找需要多少步？"
    ...
    def _generate_adaptive_questions(self, count: int, difficulty: str, language: str, subject: str, types: List[str], sub_tag: str, prompt: str) -> List[Dict[str, Any]]:
        """自适应题目生成（通用方法）"""
        questions = []
        for i in range(count):
            if subject in ['数学', 'Mathematics']:
                if difficulty == '简单':
                    ...
                    content = "Solve: ∫(x² + 3x + 2)dx from 0 to 2" if language == 'en' else "求解：∫(x² + 3x + 2)dx 从 0 到 2"
                    answer = "22/3"
                else:
                    content = "Find derivative: d/dx(x³ - 2x + 1)" if language == 'en' else "求导数：d/dx(x³ - 2x + 1)"
                    answer = "3x² - 2"
            else:
                content = f"Question about {subject} - {difficulty} level" if language == 'en' else f"关于{subject}的{difficulty}题目"
                answer = "Answer A"
    ...
    def _generate_shopping_scenario(self, count: int, difficulty: str, language: str, subject: str, prompt: str) -> List[Dict[str, Any]]:
        """生成购物场景题目 - 专业多样化版本"""
        questions = []
        import random
            ...
            'en': [
                {
                    ...
                    'items': ['apples', 'oranges', 'bananas', 'milk bottles', 'bread loaves'],
                ...
                },
                {
                    ...
                    'items': ['sneakers', 'shirts', 'backpacks', 'stationery sets', 'toys'],
                    'customers': ['a student', 'an office worker', 'a parent', 'a teenager']
                },
                {
                    ...
                    'items': ['drinks', 'snacks', 'gum packs', 'newspapers', 'water bottles'],
                ...
                },
                {
                    ...
                    'items': ['eggs', 'toothpaste', 'shampoo', 'rice bags', 'cooking oil'],
                    'customers': ['a housewife', 'a smart shopper', 'a budget-conscious person']
                },
                {
                    'type': 'bulk_purchase',
                    ...
                    'items': ['stationery', 'batteries', 'cleaning supplies', 'office supplies', 'daily necessities'],
                    'customers': ['an office manager', 'a school buyer', 'a shop owner']
                }
            ]
        }
        ...
        for i in range(count):
            ...
            if scenario['type'] in ['单价计算', 'unit_price']:
                ...
                if language == 'zh':
                    ...
                    options = [f"{correct_answer}元", f"{correct_answer+2}元", f"{correct_answer-1}元", f"{correct_answer+5}元"]
                else:
                    explanation = f"Unit price: ${template_vars['total']} ÷ {template_vars['quantity']} = ${correct_answer}"
                    options = [f"${correct_answer}", f"${correct_answer+2}", f"${correct_answer-1}", f"${correct_answer+5}"]
                ...
                if language == 'zh':
                    ...
                    options = [f"{total_cost:.0f}元", f"{total_cost*1.1:.0f}元", f"{total_cost*0.9:.0f}元", f"{total_cost*1.2:.0f}元"]
                else:
                    explanation = f"Discounted total: ${template_vars['original']} × {100-template_vars['discount']}% × {template_vars['quantity']} = ${total_cost:.0f}"
                    options = [f"${total_cost:.0f}", f"${total_cost*1.1:.0f}", f"${total_cost*0.9:.0f}", f"${total_cost*1.2:.0f}"]
                ...
                if language == 'zh':
                    ...
                    options = [f"{change}元", f"{change+10}元", f"{change-5}元", f"{change+15}元"]
                else:
                    explanation = f"Change: ${template_vars['payment']} - (${template_vars['price']} × {template_vars['quantity']}) = ${change}"
                    options = [f"${change}", f"${change+10}", f"${change-5}", f"${change+15}"]
                ...
                if language == 'zh':
                    ...
                    options = ["Option A is better", "Option B is better", "They are equal", "Cannot determine"]
    ...
    def _generate_investment_scenario(self, count: int, difficulty: str, language: str, subject: str, prompt: str) -> List[Dict[str, Any]]:
        """生成投资理财场景题目 - 多样化专业版本"""
        questions = []
        import random
        ...
        scenario_types = {
            'zh': [
                ...
                },
                {
                    ...
                    'persons': ['投资新手小王', '资深股民老李', '理财达人小张', '上班族小刘'],
                ...
                },
                {
                    ...
                    'fund_names': ['沪深300', '创业板ETF', '科技主题', '消费升级', '新能源'],
                ...
                },
                {
                    ...
                    'insurance_types': ['年金险', '万能险', '增额终身寿险', '教育金', '养老金'],
            ...
            'en': [
                {
                    ...
                    'banks': ['Bank of America', 'Wells Fargo', 'Chase Bank', 'Citibank', 'TD Bank'],
                    'persons': ['Mr. Smith', 'Ms. Johnson', 'Student Alex', 'Mrs. Davis', 'Mr. Wilson'],
                    'modes': ['fixed deposit', 'savings account', 'certificate of deposit'],
                ...
                },
                {
                    ...
                    'persons': ['novice investor John', 'experienced trader Mary', 'day trader Bob', 'long-term investor Sarah'],
                ...
                },
                {
                    ...
                    'fund_names': ['S&P 500', 'Tech ETF', 'Growth Fund', 'Value Fund', 'International'],
                    'persons': ['young professional', 'retiree', 'college graduate', 'small business owner'],
                ...
                },
                {
                    ...
                    'cities': ['New York', 'San Francisco', 'Los Angeles', 'Miami', 'Seattle'],
                    'locations': ['downtown', 'suburbs', 'waterfront', 'business district', 'residential area'],
                    'persons': ['first-time buyer', 'real estate investor', 'family upgrading', 'retiree'],
                ...
                },
                {
                    ...
                    'insurance_types': ['whole life insurance', 'annuity', 'universal life', 'education plan', 'retirement plan'],
                    'persons': ['young parent', 'middle-aged professional', 'near-retiree', 'high earner'],
        ...
        for i in range(count):
            ...
            if difficulty == '简单':
                base_values = {
                    ...
                    'period': random.choice(['1年', '2年', '3年']) if language == 'zh' else random.choice(['1 year', '2 years', '3 years']),
                ...
                base_values = {
                    ...
                    'period': random.choice(['5年', '8年', '10年', '15年']) if language == 'zh' else random.choice(['5 years', '8 years', '10 years', '15 years']),
                ...
                base_values = {
                    ...
                    'period': random.choice(['3年', '5年', '7年']) if language == 'zh' else random.choice(['3 years', '5 years', '7 years']),
            ...
            if scenario['type'] in ['银行存款', 'bank_deposit']:
                # 简单利息计算
                years = int(template_vars['period'].split('年')[0]) if language == 'zh' else int(template_vars['period'].split(' ')[0])
                ...
                if language == 'zh':
                    ...
                    options = [f"{total:.0f}元", f"{total*1.1:.0f}元", f"{total*0.9:.0f}元", f"{total*1.15:.0f}元"]
                else:
                    ...
                    options = [f"${total:.0f}", f"${total*1.1:.0f}", f"${total*0.9:.0f}", f"${total*1.15:.0f}"]
                ...
                if language == 'zh':
                    ...
                    options = [f"{profit_loss}元", f"{profit_loss+500}元", f"{profit_loss-300}元", f"{profit_loss+800}元"]
                else:
                    ...
                    options = [f"${profit_loss}", f"${profit_loss+500}", f"${profit_loss-300}", f"${profit_loss+800}"]
                    
            else:  # 其他复杂投资场景
                if language == 'zh':
                    result_value = random.randint(50000, 200000)
                    explanation = f"根据{scenario['knowledge']}进行计算，最终价值约为{result_value}元"
                    options = [f"{result_value}元", f"{result_value*1.2:.0f}元", f"{result_value*0.8:.0f}元", f"{result_value*1.5:.0f}元"]
                else:
                    result_value = random.randint(50000, 200000)
                    explanation = f"Based on {scenario['knowledge']}, the final value is approximately ${result_value}"
                    options = [f"${result_value}", f"${result_value*1.2:.0f}", f"${result_value*0.8:.0f}", f"${result_value*1.5:.0f}"]
            
            questions.append({
                'subject': subject,
                'sub_tag': f"投资理财-{scenario['type']}" if language == 'zh' else f"Investment-{scenario['type']}",
                'language': language,
                'difficulty': difficulty,
                'question_type': 'multiple_choice',
                'content': content,
    ...
    def _generate_school_scenario(self, count: int, difficulty: str, language: str, subject: str, prompt: str) -> List[Dict[str, Any]]:
        """生成学校教育场景题目 - 多样化专业版本"""
        questions = []
        import random
            ...
            'zh': [
                {
                    ...
                    'schools': ['实验小学', '希望中学', '育才学校', '明德小学', '朝阳中学'],
                    'grades': ['三年级', '四年级', '五年级', '六年级', '初一'],
                    'activities': ['美术课', '手工制作', '科学实验', '体育活动', '音乐课'],
            ...
            'en': [
                {
                    ...
                    'schools': ['Lincoln Elementary', 'Washington Middle School', 'Roosevelt High', 'Jefferson Academy'],
                    'grades': ['3rd grade', '4th grade', '5th grade', '6th grade'],
                    'activities': ['art class', 'science lab', 'PE activity', 'music lesson'],
                ...
                },
                {
                    ...
                    'schools': ['Central Elementary', 'Riverside Middle', 'Oak Hill High'],
                ...
                },
                {
                    ...
                    'schools': ['Sunshine Elementary', 'Valley Middle School', 'Hill High School'],
                    'grades': ['elementary students', 'middle school students', 'high school students'],
                ...
                },
                {
                    ...
                    'schools': ['Sports Academy', 'Regular High School', 'Athletic Center'],
                    'events': ['basketball tournament', 'soccer league', 'tennis competition'],
        ...
        for i in range(count):
            ...
            if scenario['type'] in ['教室资源分配', 'classroom_resource']:
                ...
                if language == 'zh':
                    ...
                    options = [f"{total_needed}个", f"{total_needed+20}个", f"{total_needed-15}个", f"{total_needed+35}个"]
                else:
                    ...
                    options = [f"{total_needed}", f"{total_needed+20}", f"{total_needed-15}", f"{total_needed+35}"]
                ...
                if language == 'zh':
                    ...
                    options = [f"{pass_rate:.1f}%", f"{pass_rate+10:.1f}%", f"{pass_rate-5:.1f}%", f"{pass_rate+15:.1f}%"]
                else:
                ...
                if language == 'zh':
                    ...
                    options = [f"{total_cost:.2f}元", f"{total_cost*1.2:.2f}元", f"{total_cost*0.8:.2f}元", f"{total_cost*1.5:.2f}元"]
                else:
                    ...
                    options = [f"${total_cost:.2f}", f"${total_cost*1.2:.2f}", f"${total_cost*0.8:.2f}", f"${total_cost*1.5:.2f}"]
                    
            else:  # 其他复杂场景
                if language == 'zh':
                    result = random.randint(100, 500)
                    explanation = f"根据{scenario['knowledge']}计算，结果为{result}"
                    options = [f"{result}", f"{result+50}", f"{result-30}", f"{result+80}"]
                else:
                    result = random.randint(100, 500)
    ...
    def _generate_transport_scenario(self, count: int, difficulty: str, language: str, subject: str, prompt: str) -> List[Dict[str, Any]]:
        """生成交通运输场景题目 - 多样化专业版本"""
        questions = []
        import random
            ...
            'en': [
                {
                    ...
                    'vehicles': ['high-speed train', 'car', 'airplane', 'regular train', 'bus'],
                    'origins': ['New York', 'Los Angeles', 'Chicago', 'Houston', 'Phoenix'],
                    'destinations': ['Boston', 'San Francisco', 'Detroit', 'Dallas', 'Las Vegas'],
                ...
                },
                {
                    ...
                    'persons': ['taxi driver Mike', 'car owner Sarah', 'truck driver Bill'],
                ...
                },
                {
                    ...
                    'truck_types': ['large truck', 'medium truck', 'small truck', 'refrigerated truck'],
                    'goods': ['electronics', 'food', 'construction materials', 'clothing'],
                    'origins': ['factory', 'warehouse', 'port', 'distribution center'],
                    'destinations': ['store', 'construction site', 'supermarket', 'retail outlet'],
                ...
                },
                {
                    ...
                    'locations': ['shopping mall', 'hospital', 'office building', 'tourist attraction'],
        ...
        for i in range(count):
            ...
            if scenario['type'] in ['速度时间距离', 'speed_time_distance']:
                ...
                if language == 'zh':
                    ...
                    options = [f"{speed:.1f}公里/小时", f"{speed+10:.1f}公里/小时", f"{speed-8:.1f}公里/小时", f"{speed+15:.1f}公里/小时"]
                else:
                    explanation = f"Average speed = Distance ÷ Time = {template_vars['distance']} km ÷ {template_vars['time']} hours = {speed:.1f} km/h"
                    options = [f"{speed:.1f} km/h", f"{speed+10:.1f} km/h", f"{speed-8:.1f} km/h", f"{speed+15:.1f} km/h"]
                ...
                if language == 'zh':
                    ...
                    options = [f"油费{fuel_cost}元，油耗{consumption:.1f}升/百公里", f"油费{fuel_cost+20}元，油耗{consumption+1:.1f}升/百公里", f"油费{fuel_cost-15}元，油耗{consumption-0.5:.1f}升/百公里", f"油费{fuel_cost+30}元，油耗{consumption+1.5:.1f}升/百公里"]
                else:
                    ...
                    options = [f"Cost ${fuel_cost}, {consumption:.1f} L/100km", f"Cost ${fuel_cost+20}, {consumption+1:.1f} L/100km", f"Cost ${fuel_cost-15}, {consumption-0.5:.1f} L/100km", f"Cost ${fuel_cost+30}, {consumption+1.5:.1f} L/100km"]
                ...
                if language == 'zh':
                    ...
                    options = [f"总用时{total_time}分钟，多用时{extra_time}分钟", f"总用时{total_time+10}分钟，多用时{extra_time+10}分钟", f"总用时{total_time-5}分钟，多用时{extra_time-5}分钟", f"总用时{total_time+15}分钟，多用时{extra_time+15}分钟"]
                else:
                    ...
                    options = [f"{total_time} min total, {extra_time} min extra", f"{total_time+10} min total, {extra_time+10} min extra", f"{total_time-5} min total, {extra_time-5} min extra", f"{total_time+15} min total, {extra_time+15} min extra"]
                    
            else:  # 其他复杂场景
                if language == 'zh':
                    result = random.randint(100, 1000)
                    explanation = f"根据{scenario['knowledge']}计算，结果为{result}"
                    options = [f"{result}元", f"{result+100}元", f"{result-50}元", f"{result+200}元"]
                else:
                    result = random.randint(100, 1000)
                    explanation = f"Based on {scenario['knowledge']}, the result is ${result}"
                    options = [f"${result}", f"${result+100}", f"${result-50}", f"${result+200}"]
            
            questions.append({
                'subject': subject,
                'sub_tag': f"交通运输-{scenario['type']}" if language == 'zh' else f"Transport-{scenario['type']}",
                'language': language,
                'difficulty': difficulty,
                'question_type': 'multiple_choice',
                'content': content,
    ...
    def _generate_restaurant_scenario(self, count: int, difficulty: str, language: str, subject: str, prompt: str) -> List[Dict[str, Any]]:
        """生成餐厅场景题目"""
        questions = []
        import random
        ...
        for i in range(count):
            ...
            options = [f"{pieces_per_person:.2f}块" if language == 'zh' else f"{pieces_per_person:.2f} pieces",
    ...
    def _generate_probability_statistics(self, count: int, difficulty: str, language: str, subject: str, prompt: str) -> List[Dict[str, Any]]:
        """生成概率统计题目 - 多样化专业版本"""
        questions = []
        import random
        ...
        scenario_types = {
            'zh': [
                ...
                },
                {
                    ...
                    'contexts': ['某班学生身高', '产品质量指标', '考试成绩', '测量误差', '股票收益率'],
                ...
                },
                {
                    ...
                    'condition2s': ['使用苹果手机', '开新能源车', '参加马拉松', '买电子书'],
                ...
                },
                {
                    ...
                    'data_descs': ['平均值80，方差100', '中位数5000，四分位距2000', '众数85，极差30'],
            ...
            'en': [
                {
                    ...
                    'events': ['a lottery', 'an experiment', 'a card game', 'random selection'],
                ...
                },
                {
                    ...
                    'contexts': ['Student heights in a class', 'Product quality measures', 'Test scores', 'Measurement errors'],
                ...
                },
                {
                    ...
                    'populations': ['city residents', 'college students', 'office workers', 'online shoppers'],
                    'condition1s': ['smartphones', 'cars', 'gym memberships', 'reading habits'],
                    'condition2s': ['iPhones', 'electric cars', 'marathon participation', 'e-book purchases'],
                    'knowledge': 'Conditional probability: P(A∩B) = P(A) × P(B|A)'
                },
                {
                    'type': 'descriptive_statistics',
                    'template': '{group} {measure} data: {data_desc}. What is the {statistic}?',
                    'groups': ['Class students', 'Company employees', 'Survey samples', 'Test subjects'],
                    'measures': ['age', 'salary', 'score', 'reaction time'],
                    'data_descs': ['mean 80, variance 100', 'median 5000, IQR 2000', 'mode 85, range 30'],
                    'statistics': ['standard deviation', 'coefficient of variation', 'skewness'],
                ...
                },
                {
                    ...
                    'products': ['component', 'medicine', 'food item', 'electronic device'],
        ...
        for i in range(count):
            ...
            if scenario['type'] in ['古典概率', 'classical_probability']:
                ...
                if language == 'zh':
                    ...
                    options = [f"{probability:.3f}", f"{probability*1.2:.3f}", f"{probability*0.8:.3f}", f"{probability*1.5:.3f}"]
                else:
                ...
                if language == 'zh':
                    ...
                    options = [f"{joint_prob*100:.1f}%", f"{joint_prob*100*1.3:.1f}%", f"{joint_prob*100*0.7:.1f}%", f"{joint_prob*100*1.6:.1f}%"]
                else:
            ...
            questions.append({
                'subject': subject,
                'sub_tag': f"概率统计-{scenario['type']}" if language == 'zh' else f"Statistics-{scenario['type']}",
                'language': language,
                'difficulty': difficulty,
                'question_type': 'multiple_choice',
                'content': content,
    ...
    def _create_advanced_shopping_question(self, index: int, difficulty_config: dict, language: str, subject: str, used_types: set, used_patterns: set) -> dict:
        """创建高级购物场景题目 - 强制唯一性"""
        import random
    ...
    def _generate_simple_unique_shopping(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        """简单购物题目 - 强制唯一"""
        import random
        
        scenarios = ['unit_price', 'total_cost', 'quantity_count', 'change_calc', 'comparison']
        ...
        if scenario_type == 'unit_price':
            ...
            items = ['苹果', '橘子', '葡萄', '草莓', '樱桃'][index % 5] if language == 'zh' else ['apples', 'oranges', 'grapes', 'strawberries', 'cherries'][index % 5]
            names = ['小明', '小红', '小李', '小王', '小张'][index % 5] if language == 'zh' else ['Tom', 'Mary', 'Bob', 'Alice', 'John'][index % 5]
            
            if language == 'zh':
                content = f"{names}买了{quantity}个{items}，总共花费{total}元，每个{items}的单价是多少？"
                explanation = f"单价计算：{total}元 ÷ {quantity}个 = {unit_price:.2f}元/个"
                options = [f"{unit_price:.2f}元", f"{unit_price+1:.2f}元", f"{unit_price-0.5:.2f}元", f"{unit_price+1.5:.2f}元"]
            else:
                ...
                options = [f"${unit_price:.2f}", f"${unit_price+1:.2f}", f"${unit_price-0.5:.2f}", f"${unit_price+1.5:.2f}"]
            ...
            items = ['玩具', '图书', '文具', '零食', '饮料'][index % 5] if language == 'zh' else ['toys', 'books', 'stationery', 'snacks', 'drinks'][index % 5]
            
            if language == 'zh':
                content = f"有{money}元，每个{items}价格{unit_price:.2f}元，最多能买多少个？"
                explanation = f"数量计算：{money}元 ÷ {unit_price:.2f}元 = {max_quantity}个"
                options = [f"{max_quantity}个", f"{max_quantity+1}个", f"{max_quantity-1}个", f"{max_quantity+2}个"]
            else:
                ...
                options = [f"{max_quantity}", f"{max_quantity+1}", f"{max_quantity-1}", f"{max_quantity+2}"]
    ...
    def _generate_complex_unique_shopping(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        """复杂购物题目 - 多步骤分析"""
        import random
        ...
        products = ['高端电子产品', '工业原材料', '医疗设备', '精密仪器', '专业工具'][index % 5] if language == 'zh' else ['premium electronics', 'industrial materials', 'medical equipment', 'precision instruments', 'professional tools'][index % 5]
        companies = ['科技公司Alpha', '制造企业Beta', '医疗集团Gamma', '工程公司Delta', '贸易公司Epsilon'][index % 5] if language == 'zh' else ['Tech Corp Alpha', 'Manufacturing Beta', 'Medical Group Gamma', 'Engineering Delta', 'Trading Epsilon'][index % 5]
        ...
        if language == 'zh':
            ...
            options = [f"{optimal_cost:.0f}元", f"{optimal_cost*1.08:.0f}元", f"{optimal_cost*0.94:.0f}元", f"{optimal_cost*1.15:.0f}元"]
        else:
            ...
            options = [f"${optimal_cost:.0f}", f"${optimal_cost*1.08:.0f}", f"${optimal_cost*0.94:.0f}", f"${optimal_cost*1.15:.0f}"]
    ...
    def _generate_medium_unique_shopping(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        """中等购物题目 - 两步计算"""
        import random
        ...
        products = ['运动装备', '电子产品', '家居用品', '服装配件', '美容产品'][index % 5] if language == 'zh' else ['sports equipment', 'electronics', 'home goods', 'fashion accessories', 'beauty products'][index % 5]
        occasions = ['年末促销', '会员专享', '限时特惠', '清仓甩卖', '新品上市'][index % 5] if language == 'zh' else ['year-end sale', 'member exclusive', 'flash sale', 'clearance event', 'new launch'][index % 5]
        ...
        if language == 'zh':
            ...
            options = [f"{total_cost:.1f}元", f"{total_cost*1.1:.1f}元", f"{total_cost*0.9:.1f}元", f"{total_cost*1.2:.1f}元"]
        else:
            ...
            options = [f"${total_cost:.1f}", f"${total_cost*1.1:.1f}", f"${total_cost*0.9:.1f}", f"${total_cost*1.2:.1f}"]
    ...
    def _create_advanced_statistics_question(self, index: int, difficulty_config: dict, language: str, subject: str, used_types: set, used_patterns: set) -> dict:
        """创建高级统计学题目 - 强制不同类型"""
        import random
    ...
    def _generate_simple_stats_unique(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        ...
        if scenario_type == 'coin_flip':
            ...
            topics = ['喜欢咖啡', '支持环保', '使用社交媒体', '经常运动', '购买有机食品'] if language == 'zh' else ['like coffee', 'support environment', 'use social media', 'exercise regularly', 'buy organic food']
            ...
            if language == 'zh':
                ...
                options = [f"{probability:.3f}", f"{probability*1.1:.3f}", f"{probability*0.9:.3f}", f"{probability*1.3:.3f}"]
            else:
            ...
            if language == 'zh':
                ...
                options = [f"{probability:.3f}", f"{probability*1.15:.3f}", f"{probability*0.85:.3f}", f"{probability*1.25:.3f}"]
            else:
            ...
            objects = ['球', '卡片', '学生', '产品', '票'][index % 5] if language == 'zh' else ['balls', 'cards', 'students', 'products', 'tickets'][index % 5]
            colors = ['红色', '蓝色', '绿色', '黄色', '黑色'][index % 5] if language == 'zh' else ['red', 'blue', 'green', 'yellow', 'black'][index % 5]
    ...
    def _generate_complex_stats_unique(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        ...
        diseases = ['糖尿病', '高血压', '心脏病', '肾病', '肝病'][index % 5] if language == 'zh' else ['diabetes', 'hypertension', 'heart disease', 'kidney disease', 'liver disease'][index % 5]
        ...
        if language == 'zh':
            ...
            options = [f"{positive_predictive_value:.4f}", f"{positive_predictive_value*1.3:.4f}", f"{positive_predictive_value*0.7:.4f}", f"{positive_predictive_value*1.6:.4f}"]
        else:
    ...
    def _generate_medium_stats_unique(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        ...
        conditions_a = ['使用智能手机', '有车', '大学学历', '运动习惯', '阅读习惯'][index % 5] if language == 'zh' else ['smartphone users', 'car owners', 'college graduates', 'exercise regularly', 'reading habits'][index % 5]
        conditions_b = ['使用移动支付', '买新能源车', '继续深造', '参加马拉松', '买电子书'][index % 5] if language == 'zh' else ['mobile payment', 'buy electric cars', 'pursue education', 'marathon participation', 'buy e-books'][index % 5]
        ...
        if language == 'zh':
            ...
            options = [f"{joint_probability:.4f}", f"{joint_probability*1.2:.4f}", f"{joint_probability*0.8:.4f}", f"{joint_probability*1.4:.4f}"]
        else:
    ...
    def _create_fallback_unique_question(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        """创建保底的唯一题目"""
        import random
        ...
        if language == 'zh':
            ...
            options = [f"{result}", f"{result+index+1}", f"{result-index-1}", f"{result+index*2+1}"]
        else:
    ...
    def _create_advanced_adaptive_question(self, index: int, difficulty_config: dict, language: str, subject: str, used_types: set, used_patterns: set) -> dict:
        """创建自适应专业题目"""
    ...
    def _create_gre_style_question(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        ...
        if question_type == 'data_sufficiency':
            ...
            if language == 'zh':
                ...
                options = ["A) 仅条件1充分", "B) 仅条件2充分", "C) 两个条件都需要", "D) 任一条件都充分"]
            else:
                ...
                options = ["A) Condition 1 alone is sufficient", "B) Condition 2 alone is sufficient", "C) Both conditions together are needed", "D) Either condition alone is sufficient"]
            ...
            if language == 'zh':
                ...
                options = ["A) 数量A大于数量B", "B) 数量B大于数量A", "C) 两个数量相等", "D) 无法确定大小关系"]
            else:
                ...
                options = ["A) Quantity A is greater", "B) Quantity B is greater", "C) The quantities are equal", "D) Cannot be determined"]
            ...
            if language == 'zh':
                ...
                options = [str(missing_term), str(missing_term + ratio), str(missing_term * 2), str(missing_term - ratio)]
                correct_answer = options[0]
            else:
    ...
    def _create_competition_math_question(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        """创建数学竞赛水平题目"""
        import random
        ...
        if question_type == 'combinatorics_advanced':
            ...
            if language == 'zh':
                ...
                options = [str(total_ways), str(total_ways + 100), str(total_ways - 50), str(total_ways * 2)]
                correct_answer = options[0]
            else:
    ...
    def _create_gmat_style_question(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        ...
        if question_type == 'business_optimization':
            ...
            if language == 'zh':
                ...
                options = [f"{breakeven_quantity}件和{target_quantity}件", f"{breakeven_quantity+100}件和{target_quantity+200}件", f"{breakeven_quantity-50}件和{target_quantity-100}件", f"{breakeven_quantity*2}件和{target_quantity*2}件"]
                correct_answer = options[0]
            else:
                ...
                options = [f"{breakeven_quantity} and {target_quantity} units", f"{breakeven_quantity+100} and {target_quantity+200} units", f"{breakeven_quantity-50} and {target_quantity-100} units", f"{breakeven_quantity*2} and {target_quantity*2} units"]
                correct_answer = options[0]
            ...
            if language == 'zh':
                ...
                options = [f"NPV={npv:.0f}元，{'可行' if npv > 0 else '不可行'}", f"NPV={npv*1.2:.0f}元，可行", f"NPV={npv*0.8:.0f}元，不可行", f"需要更多信息"]
                correct_answer = options[0]
            else:
                ...
                options = [f"NPV=${npv:.0f}, {'feasible' if npv > 0 else 'not feasible'}", f"NPV=${npv*1.2:.0f}, feasible", f"NPV=${npv*0.8:.0f}, not feasible", f"More information needed"]
                correct_answer = options[0]
    ...
    def _create_engineering_application_question(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        """创建工程应用题目"""
        import random
        ...
        # 工程应用题型
        engineering_types = ['structural_analysis', 'signal_processing', 'optimization_control', 'thermodynamics', 'electrical_circuits']
        ...
        if question_type == 'structural_analysis':
            ...
            if language == 'zh':
                ...
                options = [f"M={max_moment:.0f}N·m, δ={max_deflection:.2f}mm", f"M={max_moment*1.2:.0f}N·m, δ={max_deflection*1.1:.2f}mm", f"M={max_moment*0.8:.0f}N·m, δ={max_deflection*0.9:.2f}mm", "需要更多截面参数"]
                correct_answer = options[0]
            else:
                ...
                options = [f"M={max_moment:.0f}N·m, δ={max_deflection:.2f}mm", f"M={max_moment*1.2:.0f}N·m, δ={max_deflection*1.1:.2f}mm", f"M={max_moment*0.8:.0f}N·m, δ={max_deflection*0.9:.2f}mm", "More section properties needed"]
                correct_answer = options[0]
            ...
            if language == 'zh':
                ...
                if aliasing:
                    options = [f"发生混叠，混叠频率{alias_freq}Hz", "不发生混叠", f"混叠频率{alias_freq+100}Hz", "需要增加采样频率"]
                    correct_answer = options[0]
                else:
                    options = ["不发生混叠", f"发生混叠，混叠频率{signal_freq}Hz", "需要抗混叠滤波器", "采样频率过高"]
                    correct_answer = options[0]
            else:
                ...
                if aliasing:
                    options = [f"Aliasing occurs, alias frequency {alias_freq}Hz", "No aliasing", f"Alias frequency {alias_freq+100}Hz", "Need higher sampling rate"]
                    correct_answer = options[0]
                else:
                    options = ["No aliasing", f"Aliasing occurs, alias frequency {signal_freq}Hz", "Need anti-aliasing filter", "Sampling rate too high"]
                    correct_answer = options[0]
    ...
    def _create_data_science_question(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        ...
        # 数据科学题型
        ds_types = ['machine_learning', 'statistical_inference', 'algorithm_complexity', 'data_preprocessing', 'model_evaluation']
        ...
        if question_type == 'machine_learning':
            ...
            if language == 'zh':
                ...
                options = [f"训练集{train_samples}，特征子集{feature_subset}，复杂度{complexity_assessment}", f"训练集{train_samples+100}，特征子集{feature_subset+5}", f"特征子集应为{n_features//10}", "需要降维处理"]
                correct_answer = options[0]
            else:
                ...
                options = [f"Training {train_samples}, subset {feature_subset}, complexity {complexity_assessment}", f"Training {train_samples+100}, subset {feature_subset+5}", f"Subset should be {n_features//10}", "Dimensionality reduction needed"]
                correct_answer = options[0]
            ...
            if language == 'zh':
                ...
                options = [f"实际误差{actual_margin:.4f}，{'满足' if meets_requirement else '不满足'}要求", f"实际误差{actual_margin*1.2:.4f}，不满足要求", f"需要样本量{sample_size*2}", "置信度过高"]
                correct_answer = options[0]
            else:
                ...
                options = [f"Actual margin {actual_margin:.4f}, {'meets' if meets_requirement else 'does not meet'} requirement", f"Actual margin {actual_margin*1.2:.4f}, does not meet requirement", f"Need sample size {sample_size*2}", "Confidence level too high"]
                correct_answer = options[0]
    ...
    def _create_financial_modeling_question(self, index: int, difficulty_config: dict, language: str, subject: str) -> dict:
        """创建金融建模题目"""
        import random
        ...
        # 金融建模题型
        fin_types = ['options_pricing', 'portfolio_optimization', 'risk_management', 'derivative_valuation', 'credit_risk']
        ...
        if question_type == 'options_pricing':
            ...
            if language == 'zh':
                ...
                options = [f"${call_price:.2f}", f"${call_price*1.1:.2f}", f"${call_price*0.9:.2f}", f"${call_price*1.3:.2f}"]
                correct_answer = options[0]
            else:
            ...
            if language == 'zh':
                ...
                weight_a = (volatility_b**2 - correlation*volatility_a*volatility_b) / (volatility_a**2 + volatility_b**2 - 2*correlation*volatility_a*volatility_b)
                weight_b = 1 - weight_a
                portfolio_return = weight_a * expected_return_a + weight_b * expected_return_b
                portfolio_volatility = math.sqrt(weight_a**2 * volatility_a**2 + weight_b**2 * volatility_b**2 + 2*weight_a*weight_b*correlation*volatility_a*volatility_b)
                ...
                options = [f"wₐ={weight_a:.3f}, 收益{portfolio_return:.3f}, 风险{portfolio_volatility:.3f}", f"wₐ={weight_a*1.1:.3f}, 收益{portfolio_return*1.05:.3f}", f"等权重配置wₐ=0.5", "需要更多相关性数据"]
                correct_answer = options[0]
            else:
                ...
                options = [f"wₐ={weight_a:.3f}, return {portfolio_return:.3f}, risk {portfolio_volatility:.3f}", f"wₐ={weight_a*1.1:.3f}, return {portfolio_return*1.05:.3f}", f"Equal weight wₐ=0.5", "More correlation data needed"]
                correct_answer = options[0]
