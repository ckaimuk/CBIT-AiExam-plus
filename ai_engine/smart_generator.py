...
"""
            ...
            if ai_response and "content" in ai_response:
                ...
                return {
                    ...
                    "difficulty": difficulty_config["name_zh"] if config.language == Language.CHINESE else difficulty_config["name_en"],
    ...
    def _generate_ai_parameter_driven_response(self, config: GenerationConfig, index: int) -> Dict[str, Any]:
        ...
        import random
        import time
        random.seed(hash(f"{config.subject}_{config.difficulty.value}_{index}_{config.sub_domain}_{config.use_scenarios}_{time.time()}"))
        ...
        subject_specifics = {
            "数学": {
                "fields": ["代数", "分析", "几何", "拓扑", "数论", "逻辑", "组合", "概率"],
                "advanced_topics": ["范畴论", "代数几何", "微分几何", "调和分析", "表示论", "数理逻辑"],
                "research_areas": ["算术几何", "非交换几何", "高维范畴", "量子代数", "计算复杂性理论"],
                "formulas": ["$f(x) = x^2 + bx + c$", "$\\int_a^b f(x)dx$", "$\\lim_{x \\to 0} \\frac{\\sin x}{x}$", "$\\sum_{i=1}^n i^2$", "$\\frac{dy}{dx}$"]
            },
            "统计学": {
                "fields": ["描述统计", "推断统计", "回归分析", "时间序列", "多元分析", "非参数"],
                "advanced_topics": ["贝叶斯统计", "生存分析", "高维统计", "机器学习统计", "因果推断"],
                "research_areas": ["量子统计", "拓扑数据分析", "深度学习理论", "分布式统计", "隐私保护统计"],
                "formulas": ["$\\bar{x} = \\frac{1}{n}\\sum_{i=1}^n x_i$", "$P(A|B) = \\frac{P(B|A)P(A)}{P(B)}$", "$\\sigma^2 = E[(X-\\mu)^2]$", "$Z = \\frac{X-\\mu}{\\sigma}$", "$r = \\frac{\\sum(x_i-\\bar{x})(y_i-\\bar{y})}{\\sqrt{\\sum(x_i-\\bar{x})^2\\sum(y_i-\\bar{y})^2}}$"]
            },
            "物理": {
                "fields": ["力学", "热学", "电磁学", "光学", "原子物理", "固体物理"],
                "advanced_topics": ["量子力学", "相对论", "统计力学", "凝聚态物理", "粒子物理"],
                "research_areas": ["量子引力", "弦理论", "拓扑量子态", "量子信息", "宇宙学"]
            },
            "计算机科学": {
                "fields": ["算法", "数据结构", "编程", "数据库", "网络", "系统"],
                "advanced_topics": ["机器学习", "人工智能", "分布式系统", "密码学", "计算理论"],
                "research_areas": ["量子计算", "神经符号AI", "联邦学习", "同态加密", "区块链理论"]
            },
            "工程": {
                "fields": ["控制", "信号处理", "系统设计", "优化", "建模", "仿真"],
                "advanced_topics": ["自适应控制", "鲁棒控制", "最优控制", "系统识别", "智能系统"],
                "research_areas": ["自主系统", "人机协作", "边缘计算", "数字孪生", "可持续工程"]
            }
        }
        ...
        # 根据难度选择合适的主题领域
        if config.difficulty in [DifficultyLevel.HIGH_SCHOOL, DifficultyLevel.UNDERGRADUATE_BASIC]:
            topic_pool = subject_info["fields"]
        elif config.difficulty in [DifficultyLevel.UNDERGRADUATE_ADVANCED, DifficultyLevel.GRE_LEVEL, DifficultyLevel.GRADUATE_STUDY]:
        ...
        # 生成选项 - 增加多样化的理论标识和描述
        theory_labels = ["理论A", "方法B", "结论C", "模型D", "定理E", "原理F", "算法G", "策略H"]
        random.shuffle(theory_labels)
        
        approach_words = ["基于", "通过", "依据", "运用", "采用", "利用", "结合", "整合"]
    ...
    def _generate_scenario_context(self, subject: str, topic: str, difficulty: DifficultyLevel) -> str:
        ...
        if difficulty in [DifficultyLevel.HIGH_SCHOOL, DifficultyLevel.UNDERGRADUATE_BASIC]:
            level = "basic"
        elif difficulty in [DifficultyLevel.UNDERGRADUATE_ADVANCED, DifficultyLevel.GRE_LEVEL, DifficultyLevel.GRADUATE_STUDY]:
            level = "advanced"
        else:
            level = "research"
    ...
    def _generate_custom_prompt_content(self, config: GenerationConfig, selected_topic: str, current_difficulty: dict) -> str:
        """基于自定义prompt生成题目内容"""
        import random
